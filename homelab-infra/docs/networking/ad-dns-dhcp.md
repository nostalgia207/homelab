...
- **DNS** — resolves names for domain-joined machines
- **DHCP** — assigns addresses to client VMs

The domain controller sits on the management bridge (`vmbr0`), while DHCP-served client VMs sit on a separate bridge (`vmbr1`) — keeping domain/management traffic logically separated from general client traffic.
